...
if (isset($_GET['action']) && $_GET['action'] == 'tambah' && isset($_GET['id'])) {
    ...
    if ($result_item && mysqli_num_rows($result_item) == 1) {
        ...
        // Set default jumlah/kuantitas (layanan biasanya 1 unit/paket)
        $jumlah = 1; 

        // Cek apakah item sudah ada di keranjang
        if (isset($_SESSION['keranjang'][$keranjang_id])) {
            // Jika sudah ada, tambahkan kuantitasnya (misal, beli 2 paket fotografer)
        ...
        $_SESSION['success_message'] = $item['nama_layanan'] . " berhasil ditambahkan ke keranjang. Kuantitas saat ini: " . $_SESSION['keranjang'][$keranjang_id]['jumlah'];
...
// Query untuk mengambil data layanan event
...
<head>
    <meta charset="UTF-8">
    <title>Katalog Layanan Event | Sewa Event BDL</title>
                ...
                <?php if ($result_layanan && mysqli_num_rows($result_layanan) > 0): ?>
                    ...
                    <div class="product-card">
                        ...
                        <div class="product-card-body">
                            ...
                            <p class="stok" style="height: 40px; overflow: hidden; font-size: 13px;"><?php echo htmlspecialchars($layanan['deskripsi']); ?></p>
                            
                            <a href="katalog_event.php?action=tambah&id=<?php echo htmlspecialchars($layanan['id_layanan']); ?>" 
                               class="rent-btn" style="background-color: var(--success-color);">
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <p style="grid-column: 1 / -1;">Mohon maaf, saat ini belum ada layanan event yang tersedia dalam katalog.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
